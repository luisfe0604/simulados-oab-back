...
async function getENEMSimulado(req, res) {
  try {
    ...
    switch (type) {
      case 'day1':
        simulado = await service.generateENEMDay1Simulado();
        break;

      case 'day2':
        simulado = await service.generateENEMDay2Simulado();
        break;

      case 'full':
        simulado = await service.generateENEMFullSimulado();
        break;

      default:
        return res.status(400).json({
          error: 'Tipo de simulado inválido',
        });
    }
...
module.exports = { finish, list, getById, generateWrong, getENEMSimulado };
